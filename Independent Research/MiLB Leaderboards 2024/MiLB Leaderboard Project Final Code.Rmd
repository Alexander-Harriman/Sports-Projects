...
```{r}
allData <- read.csv('2024 Full MiLB May 28.csv')
...
seasonBestHitters <- topHitterSeason|>
  ...
  opt_stylize(style = 5, color = 'blue') |>
  cols_width(Name ~ px(200), Team ~ px(200), Level ~ px(100), League ~ px(200), everything() ~ px(60)) |>
  tab_footnote(footnote = 'Last Updated May 28th') |>
  gt_fmt_mlb_logo(columns = 'Org') |>
  tab_options(
    table.font.size = px(16)
  ) 
...
```{r}
write.csv(pitchingTeam, 'MiLB Team Pitching Rankings.csv')
```
