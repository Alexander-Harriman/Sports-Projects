...
# Acquire Draft Pick Information 


This will be acquired from Baseball Reference's Draft Tracker. Players taken in the top 5 rounds each year will be selected to determine how much money was spent on draft picks. The 2011 first round edition can be found here: [https://www.baseball-reference.com/draft/?year_ID=2011&draft_round=1&draft_type=junreg&query_type=year_round]


```{r}
draftPicksBonuses <- list()
k <- 0

for(i in 2011:2021){
  for(j in 1:5){
    k <- k + 1
    link <- paste0('https://www.baseball-reference.com/draft/?year_ID=',
                    i,
                    '&draft_round=',
                    j,
                    '&draft_type=junreg&query_type=year_round')
    draftPicksBonuses[k] <- link |>
      read_html() |>
      html_nodes('table') |>
      html_table()
  }
...
# Load in Season Performance Data


This will be used to determine how much value the teams earned from their draft picks.


```{r}
allSeasonData <- data.frame()

substrLast <- function(x, n){
  substr(x, nchar(x)-n+1, nchar(x))
}

setwd('E:/BBRef Season Data')
fileList <- list.files(pattern='*.xlsx')
for(i in 1:length(fileList)){
  seasonFile <- read_excel(fileList[i]) |>
    mutate(Year = substr(substrLast(basename(fileList[i]), 9), 1, 4))
  allSeasonData <- bind_rows(allSeasonData, seasonFile)
}
