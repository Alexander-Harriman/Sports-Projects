...
aaa2024 <- read.csv('2024 Full MiLB July 7.csv') |>
...
for(i in 1:dim(compPitchers)[1]){
  ...
  print(paste0(firstName1, lastName1))
  ...
  comp1RV <- savantLink1 |>
  read_html() |>
  html_nodes(xpath = '/html/body/div[2]/section/div/section/div[5]/section/div[1]/div[21]') |>
  html_table() |>
  as.data.frame() |>
  filter(X1 == 2024) |>
  filter(case_when(
    X2 == 'Split-Finger' ~ 'Splitter',
    .default = X2
  ) == compPitchers$compPitch1[i])
  
  compRV1 <- comp1RV$X5
  compRVPer1 <- comp1RV$X4
  
  
  firstName2 <- tolower(str_split(compPitchers$compName2[i], ' ')[[1]][1])
  lastName2 <- tolower(str_split(compPitchers$compName2[i], ' ')[[1]][2])
  print(paste0(firstName2, lastName2))
  ...
  filter(case_when(
    X2 == 'Split-Finger' ~ 'Splitter',
    .default = X2
  ) == compPitchers$compPitch2[i])
  
  compRV2 <- append(compRV2, comp2RV$X5)
  compRVPer2 <- append(compRVPer2, comp2RV$X4)
}


fullCompInfo <- cbind(compPitchers, compRVPer1, compRV1, compLink1, compRVPer2, compRV2, compLink2)
...
```{r}
pitcherName1 <- 'Casey Mize'
#Web Scrape Test: Get Pitch Run Values from Savant
firstName1 <- tolower(str_split(pitcherName1, ' ')[[1]][1])
lastName1 <- tolower(str_split(pitcherName1, ' ')[[1]][2])
savantTest <- paste0('https://baseballsavant.mlb.com/savant-player/',
                      firstName1,
                      '-',
                      lastName1,
                      '-',
...
colnames(runValues2024) <- c('Pitch Type', 'RV/100', 'RV', 'ID', 'Name')
