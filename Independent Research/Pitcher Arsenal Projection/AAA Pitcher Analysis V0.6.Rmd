...
aaaResults <- vaa2024 |>
  ...
  HardHitPerc = sum(ifelse(hitData.launchAngle > 0 & hitData.launchAngle < 38 & hitData.launchSpeed > 95, 1, 0), na.rm = TRUE) / sum(ifelse(details.isInPlay == TRUE, 1, 0), na.rm = TRUE),
  ChasePerc = sum(ifelse((pitchData.coordinates.pX < -.9467 | pitchData.coordinates.pX > .9467 |   pitchData.coordinates.pZ > pitchData.strikeZoneTop | pitchData.coordinates.pZ < pitchData.strikeZoneBottom) &   (details.call.description %in% c('Swinging Strike', 'Swinging Strike (Blocked)')), 1, 0)) / sum(ifelse( pitchData.coordinates.pX < -.9467 | pitchData.coordinates.pX > .9467 | pitchData.coordinates.pZ >   pitchData.strikeZoneTop | pitchData.coordinates.pZ < pitchData.strikeZoneBottom, 1, 0))
  ) |>
  ungroup() |>
...
#Third Closest Pitch 
  results3 <- which.min(dist(rbind(aaaInputs[i,], mlbInputs[-c(results, results2),]), 'euclidean', diag = FALSE, upper = FALSE)[1:nrow(mlbInputs) - 1])
