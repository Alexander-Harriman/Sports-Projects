...
* A plot with the average movement for each MLB adjusted pitch, to replace the heat map
...
mlb2024 <- read.csv('2024 Full MLB July 10.csv')
```
...
#Third Closest Pitch 
  results3 <- which.min(dist(rbind(aaaInputs[i,], mlbInputs[-c(results, results2),]), 'euclidean', diag = FALSE, upper = FALSE)[1:nrow(mlbInputs) - 2])
  
  resultInfo3 <- mlbInputs[results3,]
  
  allResultInfo3 <- cleanMLB2024 |>
    filter(IVB == resultInfo3$IVB &
           HB == resultInfo3$HB)
  
  compID3 <- append(compID3, allResultInfo3$matchup.pitcher.id)
  compName3 <- append(compName3, allResultInfo3$matchup.pitcher.fullName)
  compPitch3 <- append(compPitch3, allResultInfo3$details.type.description)
}


compPitchers <- data.frame(aaaID, aaaName, aaaPitch, compID1, compName1, compPitch1, compID2, compName2, compPitch2, compID3, compName3, compPitch3)


compPitchers
```

## Baseball Savant Run Values for Each MLB Comp
...
for(i in 1:dim(compPitchers)[1]){
  ...
  firstName3 <- tolower(str_split(compPitchers$compName3[i], ' ')[[1]][1])
  lastName3 <- tolower(str_split(compPitchers$compName3[i], ' ')[[1]][2])
  savantLink3 <- paste0('https://baseballsavant.mlb.com/savant-player/',
                        firstName3,
                        '-',
                        lastName3,
                        '-',
                        compPitchers$compID3[i],
                       '?stats=statcast-r-pitching-mlb')
  compLink3 <- append(compLink3, savantLink3)
}

fullCompInfo <- cbind(compPitchers, compLink1, compLink2, compLink3)
...
for(i in 1:2){
  ...
  mlb2023Sample <- mlb2023 |>
    filter(details.type.description == pitchType) |>
    filter(pitchData.startSpeed - 1 < cleanAAA2024[cleanAAA2024$matchup.pitcher.id == pitcherID,]$velo50 < pitchData.startSpeed + 1)
  
  
    filter(pitchData.breaks.breakVerticalInduced - 3 < cleanAAA2024[cleanAAA2024$matchup.pitcher.id == pitcherID,]$IVB < pitchData.breaks.breakVerticalInduced + 3) |>
    filter(pitchData.breaks.breakHorizontal - 3 < cleanAAA2024[cleanAAA2024$matchup.pitcher.id == pitcherID,]$HB < pitchData.breaks.breakHorizontal + 3) |>
    filter(pitchData.breaks.spinRate - 100 < cleanAAA2024[cleanAAA2024$matchup.pitcher.id == pitcherID,]$Spin < pitchData.breaks.spinRate + 100)
  
  
  infoRow <- data.frame(PitcherID = pitcherID,
                        Name = pitcherName,
                        PitchType = pitchType,
