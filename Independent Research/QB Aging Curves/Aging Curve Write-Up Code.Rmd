...
```{r}
ggplot(avgPasserRating, aes(x = Year, y = avgPR)) +
  geom_line(color = '#d50a0a', size = 0.8) +
  geom_point(color = '#013369', shape = 'diamond', size = 3) +
  xlab('Year') +
  ylab('Passer Rating') +
  ggtitle('Average Passer Rating by Year, 2006-2023') +
  theme(axis.title.y = element_text(margin = margin(r = 10), face = 'bold', size = 14),
...
for(i in 1:dim(qbData)[1]){
  ...
  qbData$averageRate[i] = avgPasserRating[avgPasserRating$Year == qbData$Year[i],]$avgPR
}
...
```{r}
table <- qbData |>
  select(playerSeasonID, GS, Att, Rate, averageRate, adjustedRate, Awards) |>
  filter(playerSeasonID %in% c('Aaron Rodgers 2014', 'Tom Brady 2016', 'Philip Rivers 2008')) |>
  mutate(averageRate = round(averageRate, 1),
         adjustedRate = round(adjustedRate, 1)) |>
  arrange(desc(adjustedRate))
...
finalTable <- table |>
  gt() |>
  tab_header(title = md('**The Evolution of Passer Rating Performance, 2008-2016**')) |>
  cols_align(align = 'center', columns = c('GS', 'Att', 'Rate', 'averageRate', 'adjustedRate', 'Awards')) |>
  cols_label(playerSeasonID = 'QB', averageRate = 'Year Average', adjustedRate = 'Rate+') |>
  opt_stylize(style = 5, color = 'blue') |>
  cols_width(playerSeasonID ~ px(160), Awards ~ px(250), everything() ~ px(75)) |>
  tab_footnote(footnote = 'Weighted Average of All QBs, Min. 100 Attempts.', locations = cells_column_labels(columns = averageRate))
...
# 2011 Aaron Rodgers Table


```{r}
rodgers <- qbData |>
  filter(Year >= 2010) |>
  mutate(allTimeRateRank = rank(-adjustedRate, ties.method = 'random'),
         allTimeQBRRank = rank(-adjustedQBR, ties.method = 'random'),
         allTimeCompRank = rank(-adjustedComp, ties.method = 'random'),
         allTimeANYARank = rank(-adjustedANYA, ties.method = 'random')) |>
  filter(playerSeasonID == 'Aaron Rodgers 2011') |>
  select(adjustedRate, allTimeRateRank, adjustedQBR, allTimeQBRRank, adjustedANYA, allTimeANYARank, adjustedComp, allTimeCompRank) |>
  mutate(adjustedRate = round(adjustedRate, 1),
         adjustedQBR = round(adjustedQBR, 1),
         adjustedANYA = round(adjustedANYA, 1),
         adjustedComp = round(adjustedComp, 1))



finalRodgers <- rodgers |>
  gt() |>
  tab_header(title = md('**2011 Aaron Rodgers Was Really Good at Football**')) |>
  cols_align(align = 'center', columns = c('adjustedRate', 'allTimeRateRank', 'adjustedQBR', 'allTimeQBRRank', 'adjustedANYA', 'allTimeANYARank', 'adjustedComp', 'allTimeCompRank')) |>
  cols_label(adjustedRate = 'Rate+', allTimeRateRank = 'Rank', adjustedQBR = 'QBR+', allTimeQBRRank = 'Rank', adjustedANYA = 'ANY/A+', allTimeANYARank = 'Rank', adjustedComp = 'Comp%+', allTimeCompRank = 'Rank') |>
  opt_stylize(style = 5, color = 'green') |>
  cols_width(everything() ~ px(75)) |>
  tab_footnote(footnote = 'Ranking out of 604 Eligible QB Seasons')
...
```{r}
ggplot(fullApp, aes(x = lastQBRPred, y = adjustedQBR)) +
  geom_point() +
  geom_smooth(method = 'lm') +
  xlab('Predicted QBR+') +
  ylab('Actual QBR+') +
...
```{r}
top5QBR <- fullApp24 |>
  mutate(QBRRank = rank(-nextQBRPred, ties.method = 'random')) |>
  filter(QBRRank <= 5) |>
  select(Player, nextQBRPred) |>
  mutate(nextQBRPred = round(nextQBRPred, 1)) |>
  arrange(desc(nextQBRPred))

colnames(top5QBR) <- c('full_name', 'nextQBRPred')
...
```{r}
team <- load_rosters(2024)

allInfo <- inner_join(top5QBR, team, by = 'full_name')

tableInfo <- allInfo |>
  select(full_name, gsis_id, nextQBRPred)
...
finalTop5 <- tableInfo |>
  gt() |>
  tab_header(title = md('**Best QBR+ Projections, 2024**')) |>
  cols_align(align = 'center', columns = c('full_name', 'gsis_id', 'nextQBRPred')) |>
  cols_label(full_name = 'Name', gsis_id = 'Image', nextQBRPred = 'QBR+') |>
  opt_stylize(style = 5, color = 'blue') |>
  cols_width(full_name ~ px(150), gsis_id ~ px(100), everything() ~ px(100)) |>
  gt_nfl_headshots(columns ='gsis_id')
...
fullApp24Final <- fullApp24 |>
  filter(Player != 'Joshua Dobbs') |>
  mutate(nextQBRPredRank = rank(-nextQBRPred, ties.method = 'random'),
         nextPasserRateRank = rank(-nextRatePred, ties.method = 'random'),
         nextCompRank = rank(-nextCompPred, ties.method = 'random'),
         nextANYAPredRank = rank(-nextANYAPred, ties.method = 'random'))
