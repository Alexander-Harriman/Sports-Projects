...
* Bounceback Innings 
* Shutdown Innings 
* Extra Bases Gained on Basepaths:
  + 1st to 3rd or 2nd to Home on Single
  + 1st to Home on Single
  + 1st to 2nd, 2nd to 3rd, 3rd to Home on Fly Ball
* 1-2-3 Innings 
...
At the end of the analysis, how these metrics relate to winning will be determined by web scraping MiLB standings and running a simple linear regression.
...
```{r}
allData <- read.csv('2024 Full MiLB June 23.csv')
...
# How Metrics Relate to Winning


## Web Scrape MiLB Standings

```{r}
library(dplyr)
allStandings <- data.frame()
leagueCodes <- c('IL', 'PCL', 'TL', 'EL', 'SOUL', 'MIDW', 'NORW', 'SALL3', 'CALL', 'CARL', 'FLOR')

for(league in leagueCodes){

milbStandingsLink <- paste0('https://www.statscrew.com/minorbaseball/standings/l-',
                            league,
                            '/y-2024')

data5 <- milbStandingsLink |>
  read_html() |>
  html_nodes('table') |>
  html_table()

standings <- as.data.frame(data5[[1]])

allStandings <- rbind(allStandings, standings)

}

cleanStandings <- allStandings[1:5] |>
  mutate(R = as.numeric(R)) |>
  filter(is.na(R) == FALSE)
...
## Baserunning vs. Winning

```{r}
colnames(cleanStandings) <- c('batting_team', 'W', 'L', 'Pct', 'R')
fullTeamBase <- inner_join(allTeamBase, cleanStandings, by = 'batting_team')

fullTeamBase <- fullTeamBase |>
...
```{r}
ggplot(fullTeamBase, aes(x = singleRate, y = Pct)) + 
  geom_point() +
  geom_smooth(method = 'lm') +
  xlab('Advance%') +
  ylab('Winning Pct') +
  ggtitle('How Advancing on Singles Affects Winning, 2024 MiLB') +
  stat_regline_equation(label.y = 0.55, aes(label = after_stat(rr.label)))
```
...
```{r}
ggplot(fullTeamBase, aes(x = doubleRate, y = Pct)) + 
  geom_point() +
  geom_smooth(method = 'lm') +
  xlab('Advance%') +
  ylab('Winning Pct') +
  ggtitle('How Advancing on Doubles Affects Winning, 2024 MiLB') +
  stat_regline_equation(label.y = 0.55, aes(label = after_stat(rr.label)))
```
...
```{r}
ggplot(fullTeamBase, aes(x = flyballRate, y = Pct)) + 
  geom_point() +
  geom_smooth(method = 'lm') +
  xlab('Advance%') +
  ylab('Winning Pct') +
  ggtitle('How Advancing on Fly Balls Affects Winning, 2024 MiLB') +
  stat_regline_equation(label.y = 0.55, aes(label = after_stat(rr.label)))
```